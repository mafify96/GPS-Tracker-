...
bool  DFRobot_SIM7000::ConnectModel(void)
{
	int count = 0;
	while(count < 3){
		if(check_send_cmd("AT+COPS=1,2,\"310410\"\r\n","OK")){
			break;
			}else{
			count++;
			delay(300);
    ...
    if(net == NB){
       ...
       if(check_send_cmd("AT+COPS=1,2,\"310410\"\r\n","OK")){
        ...
        Serial.println("pass3");
         ...
         if(check_send_cmd("AT+CGATT=1\r\n","OK")){
	     delay(300);
	     Serial.println("pass4");

		   if(check_send_cmd("AT+CSTT=\"hologram\",\"\",\"\"\r\n","OK")){
			   delay(300);
		        ...
		        Serial.println("pass6");
				 ...
				 if(check_send_cmd("AT+CGNSPWR=1\r\n","OK")){
				  ...
				  if(check_send_cmd("AT+CIPSHUT\r\n","OK")){
					  delay(300);
					  Serial.println("shut connection ok");
					  
					    if(check_send_cmd("AT+CIPMUX=0\r\n","OK")){
						    delay(300);
						    Serial.println("set to 1 okay");
				
				  if(check_send_cmd("AT+CIPRXGET=1\r\n","OK")){
					  delay(300);
					  Serial.println("get function okay");
				
     //  if(check_send_cmd("AT+CIPSTART=\"TCP\",\"things.ubidots.com\",\"9012\"\r\n","CONNECT OK")){
		    if(check_send_cmd("AT+CIPSTART=\"UDP\",\"156.110.177.12\",\"8800\"\r\n","CONNECT OK")){
	                delay(300);
	                Serial.println("connected succ!");
				return true;
            }else{
                return false;
            }
...
{
    if(mode == NB){
        if(!check_send_cmd("AT+SAPBR=3,1,\"apn\",\"hologram\"\r\n","OK")){
            return false;
        }
    }else if(mode == GPRS){
...
bool  DFRobot_SIM7000::getPosition(void)
{
	...
	if(getCommandCounter() == 4){
		position  = strstr(posBuffer,"1");
		
		memcpy(latitude,position+23, 9);
		memcpy(longitude,position+33,10);
		memcpy(year_1 , position+6 ,2);
		memcpy(month_1,position+8,2);
		memcpy(day_1 ,position+10 ,2);
		memcpy(hour_1 ,position+12 ,2);
		memcpy(minute_1 ,position+14 ,2);
		memcpy(second_1 ,position+16 ,2);
		setCommandCounter(5);
		return true;
		}else{
		return false;
...
char* DFRobot_SIM7000::getYear(void)
{
	if(getCommandCounter() >= 5){
		setCommandCounter(6);
		return year_1;
...
char* DFRobot_SIM7000::getDay(void)
{
	if(getCommandCounter() >= 5){
		setCommandCounter(6);
		return day_1;
...
char* DFRobot_SIM7000::getMonth(void)
{
	if(getCommandCounter() >= 5){
		setCommandCounter(6);
		return month_1;
...
char* DFRobot_SIM7000::getHour(void)
{
	if(getCommandCounter() >= 5){
		setCommandCounter(6);
		return hour_1;
...
char* DFRobot_SIM7000::getMinute(void)
{
	if(getCommandCounter() >= 5){
		setCommandCounter(6);
		return minute_1;
...
char* DFRobot_SIM7000::getSecond(void)
{
	if(getCommandCounter() >= 5){
		setCommandCounter(6);
		return second_1;
...
char* DFRobot_SIM7000::getLocation(void)
{
	if(getCommandCounter() >= 5){
		setCommandCounter(6);
		sprintf(SendLoc,"\{\"method\":\"location\",\"lat\":\"%s\",\"lng\":\"%s\",\"time\":\"20%s-%s-%s %s:%s:%s\",\"DeviceID\":\"Test\"\}",latitude,longitude,year_1,month_1,day_1,hour_1,minute_1,second_1);
		return SendLoc;
		}else{
		return "error";
	}
...
//////////////////////////////
bool  DFRobot_SIM7000::gettime(void)
{
	char  clockpuffer[150];
	char *timeclock;
	cleanBuffer(clockpuffer,150);
	send_cmd("AT+CGNSINF\r\n");
	readBuffer(clockpuffer,150);
	if(NULL != strstr(clockpuffer,"+CGNSINF: 1,1")){
		setCommandCounter(4);
		}else{
		return false;
	}
	if(getCommandCounter() == 4){
	timeclock  = strstr(clockpuffer,"20");
	//memcpy(year_1 ,timeclock+2 ,2);
	//memcpy(month_1,timeclock+4,2);
	//memcpy(day_1 ,timeclock+6 ,2);
	//memcpy(hour_1 ,timeclock+8 ,2);
	//memcpy(minute_1 ,timeclock+10 ,2);
	//memcpy(second_1 ,timeclock+12 ,2);
		setCommandCounter(5);
		return true;
		}else{
		return false;
